...
(-> ($= (fj 41601 :thousand :dollars) / (fj :burnrate))
    (to :days) str)
...
(-> ($= (fj :#2001-06-30) + (fj 41601 :thousand :dollars) / (fj :burnrate))
    to-date)
...
(-> ($= (fj :#now) + (fj 7 :hours) + (fj 44 :min)) to-date)
...
(-> ($= (fj :methanenergy) + (fj :h2energy)) (to :Calories) str)
...
(add-unit! :depth ($= (fj 29030 :feet) + (fj 15 :biblicalcubits) + (fj -2.4 :inches :per :year 4000 :years)))
...
(-> ($= (fj :teaspoon :water) * (fj :c) ** 2) (to :gallons :gasoline) str)
